document.getElementById("linkback").onclick = function() {
...
//        var $m2 = 14;
	...
	for(i=0; i<this.arData.length; i++){
		...
		if (this.arData[i].tt == 0) {
			var colr = "#AAAAAA";
		} else if (this.arData[i].tt.toFixed(2) < 70) {
			var colr = "#FF0000";
		} else if (this.arData[i].tt.toFixed(2) < 80){
			var colr = "#0070C0";
		} else if (this.arData[i].tt.toFixed(2) < 90){
			var colr = "#008000";
		} else {
			var colr = "#FF9900";
		}
		...
		if (this.arData[i].tt2 == 0) {
			var colr2 = "#AAAAAA";
		} else if (this.arData[i].tt2.toFixed(2) < 70) {
			var colr2 = "#FF0000";
		} else if (this.arData[i].tt2.toFixed(2) < 80){
			var colr2 = "#0070C0";
		} else if (this.arData[i].tt2.toFixed(2) < 90){
			var colr2 = "#008000";
		} else {
			var colr2 = "#FF9900";
		}
...
mapObj.createMap = function() {
	this.calcCenter();
	if(this.inType == 'div') {
//		this.home_lat = 60.698653; 
//		this.home_lng = 80.505405;
		this.zoom = 3;	
...
//	let wave = document.getElementById('id_map_wave').value;
	...
	let win = window.open(url);//, '_blank' "width=1190, height=700, location=no, status=no, toolbar=no, menubar=no, scrollbars=yes");
	win.focus();
};
...
function getChk(id){
	var req; 
	var coord = [event.pageX+"px", event.pageY+"px"];
//	var link = "/ajax/xmapex/?wave="+wave+"&id="+id;
        let url = new URL('http://localhost/RN2019_Clone/query.php');
...
//	if (obj[0].type) {
//		var iztype = (obj[0].type == 2) ? "АЗК" : "АЗС"; 
//	}
        var iztype = "АЗК";
